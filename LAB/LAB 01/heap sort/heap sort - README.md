...
## Additional Module: Heap Sort Implementation

This section documents the heap sort program, which sorts an array using a binary heap structure.

### Overview

Heap sort is a comparison-based sorting technique that uses a max heap to repeatedly extract the largest element. It is reliable, predictable, and runs in O(n log n) time.

### How It Works

1. The program reads `n` and the input array from the user.
2. It calls `heapSort`, which performs two phases:

   * **Build max heap:** Convert the array into a max heap using `heapify`.
   * **Extract elements:** Swap the root (largest) with the last element, reduce heap size, and restore heap order.
3. The final array is sorted in ascending order.
...
Provide elements when prompted.

### Example Input

```
Enter number of elements: 6
Enter 6 elements: 4 10 3 5 1 2
```
...
* **Heapify during extraction:** O(log n) per element
* **Total:** O(n log n)
* Worst, average, and best case time are the same.
...
* Heap sort is stable only with modifications; this implementation is not stable.
* It guarantees consistent performance, unlike quicksort, which can degrade on certain inputs.
